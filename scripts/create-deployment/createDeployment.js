...
program
  ...
  .requiredOption(
    ...
    "name of the environment to deploy to e.g. testing, staging",
    "testing"
  );
...
// Create a deployment directly using the GitHub API w/ a Personal Access Token
const createDeployment = ({ owner, repo, ref, environment }) => {
  ...
  .action(async (options) => {
    ...
    if (prs.length > 0) {
      await Promise.allSettled(
        prs.map((prNum) =>
          deployToInstance(`pr-${prNum}`, options.commit)
            .then(() =>
              console.log(
                `Created deployment of ${options.commit} to PR ${program.repoOwner}/${program.repoName}#${prNum}`
              )
            )
            .catch(handleError)
        )
...
program
  ...
  .action(async (options) => {
    ...
    const prData = await octokit.pulls
      .get({
        owner: program.repoOwner,
        repo: program.repoName,
        pull_number: prNum,
    ...
    if (commitPrs.includes(prNum)) {
      ...
      handleError(
        `Error: Commit ${commitSha} is not associated with PR ${program.repoOwner}/${program.repoName}#${prNum}. Is the PR closed or merged? If you specified the --commit option, check that it is correct.`
      );
    }
  });
