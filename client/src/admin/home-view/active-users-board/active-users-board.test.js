...
const activeUsers = [
  { id: 1, licences: [1, 2], user: { name: 'Test One', email: 'test1@test.com' } },
  { id: 2, licences: [3, 4], user: { name: 'Test Two', email: 'test2@test.com' } },
  { id: 3, licences: [5, 6], user: { name: 'Test Three', email: 'test3@test.com' } },
];

const pendingUsers = [
  { id: 4, licences: [7, 8], user: { name: 'Test Four', email: 'test4@test.com' } },
  { id: 5, licences: [9, 10], user: { name: 'Test Five', email: 'test5@test.com' } },
  { id: 6, licences: [11, 12], user: { name: 'Test Six', email: 'test6@test.com' } },
];
...
describe('ActiveUsersBoard', () => {
  ...
  it.each(cases)("Displays all active user's %s", (_, text) => {
    ...
    activeUsers.forEach(user => expect(getByText(user.user[text])).toBeInTheDocument());
  });
  ...
  it('Displays the licence data in the QuickView', () => {
    const { getByText } = render(
      <ActiveUsersBoard activeUsers={activeUsers} customer={customer} licenceData={licenceData} />,
    );

    expect(getByText('Active Users')).toBeInTheDocument();
    expect(getBy).toHaveDisplayValue(licenceData.active);
  ...
  describe('Displays a placeholder when there are no active users', () => {
    ...
    it.each(cases)('%s', (_, value) => {
      const { getByText } = render(
        <ActiveUsersBoard activeUsers={value} customer={customer} licenceData={licenceData} />,
      );
