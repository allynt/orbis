...
export const fetchCustomer = createAsyncThunk(
  ...
  async (user, { rejectWithValue, getState, dispatch }) => {
    await dispatch(fetchCustomerRequested());
    ...
    try {
      ...
      NotificationManager.error(
        `${status} ${message}`,
        `Fetching Customer Config Error - ${message}`,
        50000,
        () => {},
      );
...
export const updateCustomer = createAsyncThunk(
  `${name}/updateCustomer`,
  async (newCustomer, { rejectWithValue, dispatch }) => {
    await dispatch(updateCustomerRequested());
    try {
...
export const fetchCustomerUsers = createAsyncThunk(
  ...
  async (customer, { rejectWithValue, dispatch }) => {
    ...
    try {
      ...
      NotificationManager.error(
        `${status} ${message}`,
        `Fetch Customer Users Config Error - ${message}`,
        50000,
        () => {},
      );
      return rejectWithValue({ message: `${status} ${message}` });
...
export const updateCustomerUser = createAsyncThunk(
  `${name}/updateCustomerUser`,
  async (customerUser, { rejectWithValue, dispatch, getState }) => {
...
export const createCustomerUser = createAsyncThunk(
  ...
  async (fields, { getState, rejectWithValue, dispatch }) => {
    ...
    try {
      ...
      NotificationManager.error(
        `${status} ${message}`,
        `Create Customer User Config Error - ${message}`,
        50000,
        () => {},
      );
      return rejectWithValue({ message: `${status} ${message}` });
...
export const deleteCustomerUser = createAsyncThunk(
  `${name}/deleteCustomerUser`,
  async (customerUser, { getState, rejectWithValue, dispatch }) => {
