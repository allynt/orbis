...
describe('Map Event Hooks', () => {
  describe('useMapEvent Hook', () => {
    it('should call the map add event listener when added to the map', () => {
      ...
      const { rerender } = renderHook(({ map, type, cb }) => useMapEvent(map, type, cb, null), {
        initialProps: {
          map,
          type,
          cb,
  ...
  describe('useMapLayerEvent Hook', () => {
    it('should call the layer add event listener when added to the map', () => {
      ...
      const { rerender } = renderHook(({ map, type, layers, cb }) => useMapLayerEvent(map, type, layers, cb, null), {
        initialProps: {
          map,
          type,
          layers,
      ...
      layers.forEach(layer => expect(map.off).toHaveBeenCalledWith(type, layer, cb));
    });
  });
});
