...
class OrbisAnalyticsLogstashFormatter(LogstashFormatterBase):
...
class OrbisAnalyticsTCPLogstashHandler(TCPLogstashHandler):

    def __init__(self, host, port, app, instance, environment):
        super(TCPLogstashHandler, self).__init__(host, port)

        self.formatter = OrbisAnalyticsLogstashFormatter(overrides={
            "app": app,
            "instance": instance,
